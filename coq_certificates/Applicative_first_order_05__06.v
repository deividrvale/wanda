...
(to_Poly (P_const 3 * y1 + P_const 3 * y1 * (G0 ·P (y1)) + P_const 3 * (G0 ·P (y1))))
| Tfilter2  => 
 λP
λP let G1 := P_var (Vs (Vs Vz)) in
λP let y2 := P_var (Vs Vz) in
λP let y3 := P_var Vz in
(to_Poly (P_const 3 + P_const 3 * y2 + P_const 3 * y3 + (G1 ·P (P_const 0)) + P_const 3 * y3 * (G1 ·P (y3)) + P_const 3 * (G1 ·P (y2)) + P_const 3 * (G1 ·P (y3))))
| Tg  => 
 λP
(to_Poly (P_const 1))
| Tmap  => 
