...
match fn_symbols with
  | Tand  =>  form ⟶ form ⟶ form
  | TEXISTS  =>  (term ⟶ form) ⟶ form
  | TFORALL  =>  (term ⟶ form) ⟶ form
  | Tnot  =>  form ⟶ form
  | Tor  =>  form ⟶ form ⟶ form
  | TAP1 => (term ⟶ form) ⟶ term ⟶ form
end.
...
| TFORALL  => 
 λP let G0 := P_var Vz in
(to_Poly (P_const 3 + P_const 3 * (G0 ·P (P_const 0))))
| Tnot  => 
...
λP let y1 := P_var Vz in
(to_Poly (P_const 2 + P_const 2 * (G0 ·P (y1)) + P_const 3 * (G0 ·P (P_const 0))))
end.
